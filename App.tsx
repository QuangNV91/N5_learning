...
type QuizMode = 'kanji-to-meaning' | 'meaning-to-kanji';
type StudyCategory = 'all' | 'kanji' | 'verb' | 'general';
...
const App: React.FC = () => {
  ...
  const [studyCategory, setStudyCategory] = useState<StudyCategory>('all'); // State mới để lọc loại từ khi học
  ...
  const [quizMode, setQuizMode] = useState<QuizMode>('kanji-to-meaning');
  ...
  // Cập nhật logic lọc: kết hợp bài học VÀ loại từ (studyCategory)
  const filteredFlashcards = useMemo(() => {
    // Nếu đang ở màn hình chính (không chọn bài, chế độ đơn, không chọn loại chuyên sâu)
    if (selectedLesson === null && studyScope === 'single' && studyCategory === 'all') return [];

    return vocabData.filter(v => {
      const matchesLesson = studyScope === 'all' || v.lesson === selectedLesson;
      const matchesCategory = studyCategory === 'all' || v.category === studyCategory;
      return matchesLesson && matchesCategory;
    });
  }, [vocabData, selectedLesson, studyScope, studyCategory]);
  ...
  const generateQuiz = useCallback(() => {
    ...
    // Random chế độ câu hỏi
    const mode: QuizMode = Math.random() > 0.5 ? 'kanji-to-meaning' : 'meaning-to-kanji';
    setQuizMode(mode);
    ...
    // Tự động phát âm khi có câu hỏi mới (chỉ khi hiển thị tiếng Nhật)
    if (mode === 'kanji-to-meaning') {
      setTimeout(() => playAudio(correct.reading), 500);
    }
  }, [filteredFlashcards, playAudio]);

  // Reset quiz khi thay đổi bộ lọc hoặc tab
  useEffect(() => {
    setCurrentIndex(0);
    if (activeTab === 'quiz') {
      generateQuiz();
  ...
  }, [selectedLesson, studyScope, studyCategory, activeTab, generateQuiz]);
  ...
  const handleQuizAnswer = (option: Vocabulary) => {
    ...
    if (option.id === quizWord.id) {
      ...
      if (quizMode === 'meaning-to-kanji') {
        playAudio(quizWord.reading);
      }
    } else {
      const correctAnswerText = quizMode === 'kanji-to-meaning' 
        ? quizWord.meaning.toUpperCase() 
        : `${quizWord.kanji} (${quizWord.reading})`;
      
      setQuizFeedback({ correct: false, message: `SAI RỒI! ĐÁP ÁN LÀ: ${correctAnswerText}` });
      if (quizMode === 'meaning-to-kanji') {
        playAudio(quizWord.reading);
      }
    }
  ...
  const selectLesson = (lesson: number | 'all') => {
    setStudyCategory('all'); // Reset bộ lọc chuyên sâu khi chọn bài học thường
    if (lesson === 'all') {
      setStudyScope('all');
  ...
  // Hàm bắt đầu chế độ luyện tập chuyên sâu
  const startSpecialPractice = (category: StudyCategory) => {
    setSelectedLesson(null);
    setStudyScope('all');
    setStudyCategory(category);
    setActiveTab('quiz'); // Mặc định vào thẳng phần Quiz (Kiểm tra)
  };

  const handleGoHome = () => {
    setSelectedLesson(null);
    setStudyScope('single');
    setStudyCategory('all'); // Reset về trạng thái ban đầu
  };
  ...
  const currentLessonTitle = useMemo(() => {
    if (studyCategory === 'kanji') return "THỬ THÁCH KANJI N5";
    if (studyCategory === 'verb') return "THỬ THÁCH ĐỘNG TỪ";
  ...
  }, [selectedLesson, studyScope, studyCategory]);

  const shouldShowFurigana = (vocab: Vocabulary | null) => {
    if (!vocab) return false;
    return vocab.category !== 'kanji' && vocab.kanji !== vocab.reading;
  };

  // Màn hình chọn bài (Home)
  if (selectedLesson === null && studyScope === 'single' && studyCategory === 'all') {
    return (
        ...
        <div className="max-w-5xl w-full space-y-12 py-10">
          ...
          <div className="space-y-6 pt-6">
             <div className="flex items-center gap-4">
                ...
                <span className="text-xs font-black uppercase tracking-[0.3em] text-slate-400">LUYỆN TẬP CHUYÊN SÂU</span>
                <div className="h-0.5 bg-black flex-1"></div>
             </div>
             
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <button 
                  onClick={() => startSpecialPractice('kanji')}
                  className="py-8 bg-[#F9F6F0] text-black border-4 border-black shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] hover:bg-black hover:text-white transition-all font-black text-xl uppercase tracking-[0.2em] flex items-center justify-center gap-4 group"
                >
                  <span className="text-3xl group-hover:scale-110 transition-transform">🈳</span>
                  <span>KIỂM TRA KANJI N5</span>
                </button>
                <button 
                  onClick={() => startSpecialPractice('verb')}
                  className="py-8 bg-[#F9F6F0] text-black border-4 border-black shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] hover:bg-black hover:text-white transition-all font-black text-xl uppercase tracking-[0.2em] flex items-center justify-center gap-4 group"
                >
                  <span className="text-3xl group-hover:scale-110 transition-transform">🏃</span>
                  <span>KIỂM TRA ĐỘNG TỪ</span>
                </button>
             </div>
             ...
             <div className="pt-2">
              <button 
                ...
                className="w-full py-8 bg-[#B11116] text-white border-4 border-black shadow-[10px_10px_0px_0px_rgba(0,0,0,1)] hover:bg-[#8E0E12] transition-all font-black text-2xl uppercase tracking-[0.3em] flex items-center justify-center gap-6"
              >
            ...
            {quizWord ? (
              <div className="bg-white border-4 border-black p-10 shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] space-y-10 relative">
                ...
                <div className="text-center relative">
                   <div className="text-xs text-slate-400 font-black uppercase tracking-[0.4em] mb-4">
                     {quizMode === 'kanji-to-meaning' ? 'MẶT CHỮ HÁN' : 'Ý NGHĨA'}
                   </div>
                   
                   <div className="flex flex-col items-center justify-center">
                    {/* Hiển thị Furigana cho câu hỏi nếu cần */}
                    {quizMode === 'kanji-to-meaning' && shouldShowFurigana(quizWord) && (
                      <div className="text-3xl text-slate-500 font-bold mb-2">{quizWord?.reading}</div>
                    )}

                    <div className="flex items-center gap-4">
                      <div className={`${quizMode === 'kanji-to-meaning' ? 'text-8xl' : 'text-5xl text-[#B11116] uppercase'} font-black text-black`}>
                        {quizMode === 'kanji-to-meaning' ? quizWord?.kanji : quizWord?.meaning}
                      </div>
                      {/* Chỉ hiện loa khi đang hỏi tiếng Nhật hoặc đã trả lời xong */}
                      {(quizMode === 'kanji-to-meaning' || quizAnswered) && (
                        <button 
                   ...
                   <div className="text-slate-400 font-bold italic h-8 tracking-widest uppercase mt-4">
                    {quizAnswered ? quizWord?.reading : (quizMode === 'kanji-to-meaning' ? '???' : '')}
                   </div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {quizOptions.map((opt) => (
                    ...
                    >
                      ...
                      {quizMode === 'kanji-to-meaning' ? (
                        <span>{opt.meaning.toUpperCase()}</span>
                      ) : (
                        <div className="flex flex-col items-start">
                          {/* Hiển thị Furigana cho đáp án nếu cần */}
                          {shouldShowFurigana(opt) && (
                            <span className="text-xs text-slate-500 font-bold mb-1 opacity-70 group-hover:text-white">{opt.reading}</span>
                          )}
                          <span>{opt.kanji}</span>
                        </div>
                      )}
