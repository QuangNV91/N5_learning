...
export const Flashcard: React.FC<FlashcardProps> = ({ vocab, onExplain }) => {
  ...
  // Logic hiển thị Furigana: Không phải loại 'kanji' VÀ mặt chữ khác cách đọc
  const showFurigana = vocab.category !== 'kanji' && vocab.kanji !== vocab.reading;
      ...
      >
        <div className={`relative w-full h-full transition-transform duration-500 preserve-3d ${isFlipped ? 'rotate-y-180' : ''}`}>
          ...
          <div className="absolute inset-0 backface-hidden bg-white flex flex-col items-center justify-center p-8 border-4 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
            ...
            {/* Furigana Hint */}
            {showFurigana && (
              <span className="text-2xl text-slate-500 font-bold mb-[-0.5rem] tracking-wider">{vocab.reading}</span>
            )}
            
            <h2 className="text-8xl font-black text-black tracking-tighter text-center leading-tight">{vocab.kanji}</h2>
